          ...
          <div class="card card-border-color card-border-color-primary">
            <div class="card-body">
             @foreach ($detail as $item)
              <form action="{{url('projects/update/'.$item->KodeProject)}}" method="POST" >
                @csrf
                <div class="form-group row">
                  <label class="col-12 col-sm-3 col-form-label text-sm-right" for="">Kode PT</label>
                  <div class="col-12 col-sm-8 col-lg-6">
                    <input class="form-control" id="KodePT" type="text" name="KodePT" disabled value="{{$item->KodePT}}">
                    <input class="form-control" id="KodePT" type="hidden" name="KodePT"  value="{{$item->KodePT}}">
                  </div>
                </div>
                <div class="form-group row">
                  <label class="col-12 col-sm-3 col-form-label text-sm-right" for="">Kode Project</label>
                  <div class="col-12 col-sm-8 col-lg-6">
                    <input class="form-control" id="KodeProject" type="text" name="KodeProject" disabled value="{{$item->KodeProject}}">
                  </div>
                </div>
                <div class="form-group row">
                  <label class="col-12 col-sm-3 col-form-label text-sm-right" for="">Nama Project</label>
                  <div class="col-12 col-sm-8 col-lg-6">
                    <input class="form-control" id="NamaProject" type="text" name="NamaProject" value="{{$item->NamaProject}}">
                  </div>
                </div>
                <div class="form-group row">
                  <label class="col-12 col-sm-3 col-form-label text-sm-right" for=""></label>
                  <div class="col-12 col-sm-8 col-lg-6">
                    <button class="btn btn-space btn-success active" type="submit"><i class="icon icon-left mdi mdi-cloud-done"></i>Save Change</button>
